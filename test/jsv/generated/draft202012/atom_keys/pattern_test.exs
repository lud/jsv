...
defmodule JSV.Generated.Draft202012.AtomKeys.PatternTest do
  ...
  describe "pattern with Unicode property escape requires unicode mode" do
    setup do
      json_schema = %JSV.Schema{
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        type: "string",
    ...
    test "ASCII letters match", x do
      data = "Hello"
      expected_valid = true
      JsonSchemaSuite.run_test(x.json_schema, x.schema, data, expected_valid, print_errors: false)
    end

    test "Non-ASCII letters match", x do
      data = "π"
      expected_valid = true
      JsonSchemaSuite.run_test(x.json_schema, x.schema, data, expected_valid, print_errors: false)
    end

    test "Digits do not match", x do
      data = "123"
      expected_valid = false
      JsonSchemaSuite.run_test(x.json_schema, x.schema, data, expected_valid, print_errors: false)
    end
